...
export default function DocsArchive() {
  ...
  const [data, setData] = useState<any>(null); // 🚀 LIVE STATE

  // 🚀 FETCH LIVE DATA: This bypasses the Next.js static JSON cache
  useEffect(() => {
    fetch('/api/content')
      .then(res => res.json())
  ...
  // Show a clean loading state while fetching the latest DB
  if (!data) return (
  ...
  // 🚀 SECURITY FILTER: Only show documents marked as Published
