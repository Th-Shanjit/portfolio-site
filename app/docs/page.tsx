...
export default function DocsArchive() {
  ...
  const [data, setData] = useState<any>(null); // 🚀 Real-time State

  // 🚀 FETCH LIVE DATA: Bypasses static caching
  useEffect(() => {
    fetch('/api/content')
      .then(res => res.json())
          ...
          {filteredDocs.length > 0 ? (
            filteredDocs.map((doc: any, index: number) => (
              <ScrollReveal key={doc.id} delay={index * 100}>
                <Link href={`/docs/${doc.id}`} className="group block bg-white/40 border border-zinc-200 hover:border-zinc-300 rounded-[2rem] p-6 md:p-8 transition-all duration-300 hover:shadow-xl hover:bg-white/70 backdrop-blur-md">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex-1">
                      ...
                      <h2 className="text-2xl font-semibold text-zinc-900 tracking-tight mb-2 group-hover:text-zinc-700 transition-colors">{doc.title}</h2>
                      <p className="text-sm text-zinc-500 font-light line-clamp-2 md:line-clamp-1 pr-4">{doc.content[0]?.replace(/^### /, '') || "Read the full architectural breakdown."}</p>
                    </div>
            ...
            <ScrollReveal>
              <div className="text-center py-20 bg-white/30 border-2 border-zinc-200 border-dashed rounded-[2.5rem] backdrop-blur-sm">
                ...
                {(searchTerm !== '' || activeFilter !== 'All') && (
                  <button onClick={() => { setSearchTerm(''); setActiveFilter('All'); }} className="mt-6 text-xs font-bold uppercase tracking-widest text-zinc-900 hover:text-zinc-500 transition-colors underline underline-offset-4">Clear all filters</button>
                )}
              </div>
            </ScrollReveal>
          )}
