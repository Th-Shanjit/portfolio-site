...
export default function FeaturesSection() {
  useReveal();
  return (
    <section id="features" className="bg-[#F6F2EB] text-[#0B1825] py-14 sm:py-20 md:py-24 lg:py-28">
      <div className="max-w-[1180px] mx-auto w-full px-[clamp(16px,5vw,48px)]">
        <div className="pl-reveal max-w-[42rem] mb-8 md:mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="col-span-full bg-[#0B1825] text-[#EDE8DB] rounded-2xl p-6 sm:p-8 md:p-10 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 items-center border border-transparent min-w-0">
            <div className="min-w-0">
              <span className="font-mono text-[11px] sm:text-[12px] text-[#F0A535] tracking-[0.18em] uppercase">
                Extraction approach
              </span>
              <h3 className="font-serif font-bold text-[22px] sm:text-[26px] md:text-[30px] leading-tight mt-3 mb-3">
                Gemini Vision workflow. Not <em className="italic">blind OCR.</em>
              </h3>
              <p className="font-sans text-[15px] sm:text-[16px] text-[#EDE8DB]/60 leading-[1.7] max-w-[28rem]">
                AI handles the first draft of the structure; the teacher stays in control before the
                PDF is generated.
              </p>
            </div>
            <div className="flex flex-wrap gap-2 min-w-0">
              {ACCURACY_TAGS.map((tag) => (
                <span
                  key={tag}
                  className="font-mono text-[12px] sm:text-[13px] text-[#EDE8DB] bg-white/[0.06] border border-white/[0.08] rounded-full px-3 py-1.5"
                >
                  {tag}
                </span>
              ))}
